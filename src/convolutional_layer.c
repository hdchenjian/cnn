...
convolutional_layer *make_convolutional_layer(int h, int w, int c, int n, int size, int stride, ACTIVATION activation)
{
    ...
    layer->size = size;
    layer->stride = stride;
    layer->weights = calloc(n, sizeof(image));
    layer->weight_updates = calloc(n, sizeof(image));
    layer->weight_momentum = calloc(n, sizeof(image));
    layer->biases = calloc(n, sizeof(float));
    layer->bias_updates = calloc(n, sizeof(float));
    layer->bias_momentum = calloc(n, sizeof(float));
    float scale = 1./(size*size*c);
    for(int i = 0; i < n; ++i){
        layer->weights[i] = make_random_kernel(size, c, scale);
        layer->weight_updates[i] = make_random_kernel(size, c, 0);
        layer->weight_momentum[i] = make_random_kernel(size, c, 0);
    }
...
void forward_convolutional_layer(const convolutional_layer *layer, float *in)
{
    ...
    int i,j;
    for(i = 0; i < layer->n; ++i){
        convolve(input, layer->weights[i], layer->stride, i, output);
    }
    //float max = 0.0F;
    //float min = 0.0F;
...
void backward_convolutional_layer(const convolutional_layer *layer, float *delta)
{
    ...
    for(i = 0; i < layer->n; ++i){
        back_convolve(in_delta, layer->weights[i], layer->stride, i, out_delta);
    }
}
...
void learn_convolutional_layer(const convolutional_layer *layer, float *input)
{
    ...
    for(int i = 0; i < layer->n; ++i){
        kernel_update(in_image, layer->weight_updates[i], layer->stride, i, out_delta);
...
void update_convolutional_layer(const convolutional_layer *layer, float learning_rate, float momentum, float decay)
{
    ...
    for(i = 0; i < layer->n; ++i){
        ...
        int pixels = layer->weights[i].h*layer->weights[i].w*layer->weights[i].c;
        for(j = 0; j < pixels; ++j){
            layer->weight_momentum[i].data[j] = learning_rate*(layer->weight_updates[i].data[j] - decay*layer->weights[i].data[j])
                                                + momentum*layer->weight_momentum[i].data[j];
            layer->weights[i].data[j] += layer->weight_momentum[i].data[j];
            //layer->weights[i].data[j] = constrain(layer->weights[i].data[j], 1.);
        }
        zero_image(layer->weight_updates[i]);
    }
}
