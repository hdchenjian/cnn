...
gru_layer *make_gru_layer(int batch, int inputs, int outputs, int steps, int batch_normalize)
{
    ...
    fprintf(stderr, "\t");
    l->uz = make_connected_layer(inputs, outputs, connected_layer_batch, steps, activation, weight_normalize,
                                bias_term, lr_mult, lr_decay_mult, bias_mult, bias_decay_mult, weight_filler,
                                sigma, batch_normalize);
    l->uz->batch = batch;

    fprintf(stderr, "\t");
    l->ur = make_connected_layer(inputs, outputs, connected_layer_batch, steps, activation, weight_normalize,
                                bias_term, lr_mult, lr_decay_mult, bias_mult, bias_decay_mult, weight_filler,
                                sigma, batch_normalize);
    l->ur->batch = batch;

    fprintf(stderr, "\t");
    l->uh = make_connected_layer(inputs, outputs, connected_layer_batch, steps, activation, weight_normalize,
                                bias_term, lr_mult, lr_decay_mult, bias_mult, bias_decay_mult, weight_filler,
                                sigma, batch_normalize);
    l->uh->batch = batch;
    
    fprintf(stderr, "\t");
    l->wz = make_connected_layer(outputs, outputs, connected_layer_batch, steps, activation, weight_normalize,
                                bias_term, lr_mult, lr_decay_mult, bias_mult, bias_decay_mult, weight_filler,
                                sigma, batch_normalize);
    l->wz->batch = batch;
    ...
    fprintf(stderr, "\t");
    l->wr = make_connected_layer(outputs, outputs, connected_layer_batch, steps, activation, weight_normalize,
                                bias_term, lr_mult, lr_decay_mult, bias_mult, bias_decay_mult, weight_filler,
                                sigma, batch_normalize);
    l->wr->batch = batch;

    fprintf(stderr, "\t");
    l->wh = make_connected_layer(outputs, outputs, connected_layer_batch, steps, activation, weight_normalize,
                                bias_term, lr_mult, lr_decay_mult, bias_mult, bias_decay_mult, weight_filler,
                                sigma, batch_normalize);
    l->wh->batch = batch;
...
void backward_gru_layer(gru_layer *l, float *input, float *delta, int test)
{
    ...
    for(int i = l->steps-1; i >= 0; --i) {
        ...
        copy_cpu(l->outputs*l->batch, l->state, 1, l->forgot_state, 1);
        mul_cpu(l->outputs*l->batch, l->r_cpu, 1, l->forgot_state, 1);
        backward_connected_layer(l->wh, l->forgot_state, l->forgot_delta, test, 0);
        ...
        backward_connected_layer(l->wr, l->state, prev_delta, test, 0);
        backward_connected_layer(l->wz, l->state, prev_delta, test, 1);

        backward_connected_layer(l->uh, input, delta, test, 0);
        backward_connected_layer(l->ur, input, delta, test, 1);
        backward_connected_layer(l->uz, input, delta, test, 1);
    }
...
void backward_gru_layer_gpu(gru_layer *l, float *input, float *delta, int test)
{
    ...
    printf("welll\n");
        ...
        copy_gpu(l->outputs*l->batch, l->state_gpu, 1, l->forgot_state_gpu, 1);
        mul_gpu(l->outputs*l->batch, l->r_gpu, 1, l->forgot_state_gpu, 1);
        backward_connected_layer_gpu(l->wh, l->forgot_state_gpu, l->forgot_delta_gpu, test, 0);
        ...
        backward_connected_layer_gpu(l->wr, l->state_gpu, prev_delta_gpu, test, 0);
        backward_connected_layer_gpu(l->wz, l->state_gpu, prev_delta_gpu, test, 1);

        backward_connected_layer_gpu(l->uh, input, delta, test, 0);
        backward_connected_layer_gpu(l->ur, input, delta, test, 1);
        backward_connected_layer_gpu(l->uz, input, delta, test, 1);
    }
