...
__global__ void weight_normalize_kernel(int inputs, int outputs, float *x)
{
    int index = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if (index >= outputs) return;
    float sum = 1e-6;
    for(int j = 0; j < inputs; j++){
        float temp = x[index * inputs + j];
        sum += temp * temp;
    }
...
extern "C" void weight_normalize_gpu(int inputs, int outputs, float *x)
{
    weight_normalize_kernel<<<cuda_gridsize(outputs), BLOCK>>>(inputs, outputs, x);
    check_error(cudaPeekAtLastError());
}
...
__global__ void is_max_gpu_kernel(int batch, int inputs, float *output_gpu, int *truth_label_index_gpu, int *is_not_max)
{
    int index = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    int b = index / inputs;
    if(b >= batch) return;
